...
# NEW: Parameters for synthetic data handling
# Set to control how much synthetic data to include (0.2 = use 20% of synthetic samples)
SYNTHETIC_RATIO = 0.2  
# Set weight for synthetic samples (1.0 = equal weight as real samples, 0.5 = half weight)
SYNTHETIC_WEIGHT = 0.5  

# 1. Load data and create augmented training set with controlled synthetic ratio
print("Loading and preparing data with controlled synthetic injection...")
...
# Create labels (all synthetic samples are fraud=1)
y_synthetic = pd.Series(np.ones(len(X_synthetic), dtype=int))

# NEW: Calculate how many synthetic samples to use based on SYNTHETIC_RATIO
real_fraud_count = y_train_orig.sum()
synthetic_to_use = int(real_fraud_count * SYNTHETIC_RATIO)
print(f"Real fraud samples in original data: {real_fraud_count}")
print(f"Using {synthetic_to_use} synthetic samples ({SYNTHETIC_RATIO*100:.1f}% of real fraud)")

# If we want fewer synthetic samples than available, sample randomly
if synthetic_to_use < len(X_synthetic):
    # Sample without replacement
    synthetic_indices = np.random.choice(len(X_synthetic), synthetic_to_use, replace=False)
    X_synthetic_subset = X_synthetic.iloc[synthetic_indices].reset_index(drop=True)
    y_synthetic_subset = y_synthetic.iloc[synthetic_indices].reset_index(drop=True)
else:
    X_synthetic_subset = X_synthetic
    y_synthetic_subset = y_synthetic

# Combine original and synthetic data
X_train_combined = pd.concat([X_train_orig, X_synthetic_subset], ignore_index=True)
y_train_combined = pd.concat([y_train_orig, y_synthetic_subset], ignore_index=True)

# NEW: Create sample weights
sample_weights = np.ones(len(X_train_combined))
# Identify synthetic samples (they're the last synthetic_to_use samples)
synthetic_indices = list(range(len(X_train_orig), len(X_train_combined)))
# Set lower weights for synthetic samples
sample_weights[synthetic_indices] = SYNTHETIC_WEIGHT
print(f"Applied sample weight of {SYNTHETIC_WEIGHT} to {len(synthetic_indices)} synthetic samples")
...
print(f"Combined training data: {X_train_combined.shape}, Validation data: {X_val.shape}, Test data: {X_test.shape}")
print(f"Fraud ratio in combined training: {y_train_combined.mean():.6f}")
print(f"Original fraud ratio (reference): {y_train_orig.mean():.6f}")
...
# 3. Train the model
print("Training XGBoost model on controlled synthetic data...")
...
# For XGBoost versions that support sample_weight in fit
eval_set = [(X_train_combined, y_train_combined), (X_val, y_val)]
model.fit(X_train_combined, y_train_combined, 
          sample_weight=sample_weights,  # NEW: Using sample weights
          eval_set=eval_set,
          verbose=True)
...
final_model.fit(X_train_combined, y_train_combined, sample_weight=sample_weights)  # NEW: Using sample weights
model = final_model
...
print("\n======= Test Set Performance with Controlled Synthetic Data =======")
...
plt.barh(X_train_combined.columns[sorted_idx], feature_importance[sorted_idx])
plt.title("Feature Importance (Controlled Synthetic Model)")
...
plt.title('Confusion Matrix (Controlled Synthetic Model)')
plt.ylabel('True Label')
plt.xlabel('Predicted Label')

plt.tight_layout()
plt.savefig('xgboost_controlled_synthetic_results.png', dpi=300)
plt.show()

# 6. Save the model
model.save_model('Models/xgboost_controlled_synthetic.json')
print("Model saved to 'Models/xgboost_controlled_synthetic.json'")

# Optional: Save prediction probabilities for later analysis
pd.DataFrame({
    'true_label': y_test,
    'fraud_probability': y_pred_proba
}).to_csv('Results/controlled_synthetic_predictions.csv', index=False)
...
metrics = {
    ...
    'synthetic_configuration': {
        ...
        'synthetic_samples_used': synthetic_to_use
    }
}

# Save to JSON file
with open('Results/controlled_synthetic_metrics.json', 'w') as f:
...
roc_data.to_csv('Results/controlled_synthetic_roc_curve_data.csv', index=False)
...
pr_data.to_csv('Results/controlled_synthetic_pr_curve_data.csv', index=False)

# 7. Add comparison to baseline metrics if available
try:
    with open('Results/model_metrics.json', 'r') as f:
        baseline_metrics = json.load(f)
    
    print("\n======= Comparison with Baseline Model =======")
    print(f"Metric             | Baseline   | Controlled Synthetic | Change")
    print(f"--------------------|------------|---------------------|-------")
    ...
    try:
        ...
        controlled_pr = pd.read_csv('Results/controlled_synthetic_pr_curve_data.csv')
        
        plt.plot(baseline_pr['recall'], baseline_pr['precision'], label='Baseline')
        plt.plot(controlled_pr['recall'], controlled_pr['precision'], label='Controlled Synthetic')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title('Precision-Recall Curve Comparison')
        plt.legend()
    except Exception as e:
        print(f"Could not create PR curve comparison: {e}")
    ...
    controlled_values = [metrics[m] for m in metrics_to_plot]
    
    x = np.arange(len(labels))
    width = 0.35
    
    plt.bar(x - width/2, baseline_values, width, label='Baseline')
    plt.bar(x + width/2, controlled_values, width, label='Controlled Synthetic')
    ...
    plt.tight_layout()
    plt.savefig('controlled_synthetic_comparison.png', dpi=300)
    plt.show()
