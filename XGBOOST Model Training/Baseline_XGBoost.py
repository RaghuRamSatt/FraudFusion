...
print(f"Fraud ratio in training: {y_train.mean():.6f}")
...
plt.tight_layout()
plt.savefig('xgboost_baseline_results.png', dpi=300)
plt.show()
...
}).to_csv('Results/baseline_predictions.csv', index=False)
...
roc_data.to_csv('Results/roc_curve_data.csv', index=False)
...
pr_data.to_csv('Results/pr_curve_data.csv', index=False)